...
if ($routename == "updatepersonalinfo") {
    ...
    PDO_Execute("update personalinfo set fathername=?,city=?,coname=?,corealtion=?,dob=?,address=?,pan=?,acno=?,ifsc=? where userid=?",
...
if ($routename == "downlinestatus") {
    try {
        ...
        $limit = "";
		$result=array();
       	 $teamrows = PDO_FetchAll("SELECT count(*) pvalue,side from upliners where upliners.upliners= ?  group by upliners.side ",[$userid]);
