...
$limit = '';
$data = isset($json_data[2][0]) ? $json_data[2][0] : null;

// print_r($data); echo 'AND ';
// print_r($json_data);
...
if ($routename == "updaterewardstatuscomplete") {
    ...
    for ($i; $i <= 4; $i++) {
        ...
        PDO_Execute("UPDATE lifetimebonanzaachivers SET lifetimebonanzaachivers.achieved = 1 WHERE lifetimebonanzaachivers.bvqualified = 1 AND teamqualified = 1 AND achieved IS NULL");
...
if ($routename == "downlinelist") {
    ...
    try {
        ...
        //     // Explicitly extract required variables to avoid undefined index or null issues
        //     // $findparam = isset($data["findparam"]) ? $data["findparam"] : null;
        //     $fromdate = isset($data["fromdate"]) ? $data["fromdate"] : null;
        //     $uptodate = isset($data["uptodate"]) ? $data["uptodate"] : null;
        ...
        //     //     $qparam .= " AND ";
        //     // }
        //     // $qparam .= "upliners.udoa > ?";
        //     // if ($qparam === "") {
        //     //     $qparam = " WHERE ";
        //     // } else {
        //     //     $qparam .= " AND ";
        //     // }
        //     // $qparam .= "upliners.udoa < DATE_ADD(?, INTERVAL 1 DAY)";
        //     // if ($qparam === "") {
        //     //     $qparam = " WHERE ";
        //     // } else {
        //     //     $qparam .= " AND ";
        //     // }
        //     // $qparam .= "upliners.upliners=?";
        //     // $paramarr[] = $fromdate;
        //     // $paramarr[] = $uptodate;
        //     // $paramarr[] = $userid;


        //     $SELECT_query = "SELECT personalinfo.userid, personalinfo.name, personalinfo.doj, upliners.doa, plans.planname, `binary`.sid, `binary`.`uid`, plans.planvalue FROM upliners JOIN personalinfo ON personalinfo.userid = upliners.userid JOIN `binary` ON `binary`.userid = upliners.userid JOIN plans ON plans.planid = `binary`.planid ORDER BY `personalinfo`.userid";

        //     $count_query = "SELECT COUNT(*) AS tcount, SUM(upliners.upliners) AS tpvalue
        //         FROM upliners
        //         JOIN personalinfo ON personalinfo.userid = upliners.userid
        //         JOIN `binary` ON `binary`.userid = upliners.userid
        //         JOIN plans ON plans.planid = `binary`.planid";
        //     $limit = " LIMIT " . intval(($page - 1) * $pagesize) . " , " . $pagesize;
        ...
        $limit = isset($data["page"]) ? " LIMIT " . intval(($data['page'] - 1) * $data['pagesize']) . "," . $data['pagesize'] : '';

        $q = new \stdClass();
        // $q->result = $pdo->query($SELECT_query . $limit)->fetchAll(PDO::FETCH_ASSOC);
        // $q->totalcount = $pdo->query($count_query)->fetch(PDO::FETCH_ASSOC);
...
//             upliners.doa, plans.planname,`binary`.sid,`binary`.`uid`, plans.planvalue,ifnull(`binary`.isapproved,0) isapproved from upliners
//             join personalinfo on personalinfo.userid = upliners.userid
//             join `binary`on `binary`.userid= upliners.userid
...
if ($routename == "memberslist") {
    try {
        ...
        // $SELECT_query = "SELECT personalinfo.userid, personalinfo.name, personalinfo.doj, `binary`.doa, ifnull(`binary`.isapproved,0) isapproved,users.username,users.password , `binary`.sid,users.password, plans.planname, plans.planid,personalinfo.mobile
        //      From  personalinfo
        ...
        // JOIN users ON users.username=personalinfo.userid
        // JOIN `binary` ON `binary`.userid= users.username  JOIN plans ON plans.planid=`binary`.planid " . $qparam . " ORDER BY `personalinfo`.doj DESC " . $limit;
        $SELECT_query = "SELECT personalinfo.userid, personalinfo.name, personalinfo.doj, `binary`.doa, ifnull(`binary`.isapproved,0) isapproved,users.username,users.password , `binary`.sid,users.password,personalinfo.mobile FROM personalinfo JOIN users ON users.username=personalinfo.userid JOIN `binary` ON `binary`.userid= users.username ORDER BY `personalinfo`.doj DESC";
        ...
        FROM  personalinfo
        JOIN users ON users.username=personalinfo.userid 
        JOIN `binary` ON `binary`.userid= users.username ";
...
if ($routename == "activatedlist") {
    ...
    try {
        ...
        $SELECT_query = "SELECT personalinfo.userid, personalinfo.name, personalinfo.doj, `binary`.doa, ifnull(`binary`.isapproved,0) isapproved,users.username,users.password ,users.password, plans.planname, mobile From personalinfo join users on users.username=personalinfo.userid Join `binary`on `binary`.userid= users.username join plans on plans.planid=`binary`.planid order by `binary`.doa desc";
        ...
        // $q->totalcount = PDO_FetchOne($count_query);
        // $q->lastpayoutdate = PDO_FetchOne("select date_add(upperdate,interval 1 day) from payout order by payoutid desc limit 1");
        $q->lastpayoutdate = PDO_FetchOne("SELECT DATE_ADD(payoutdate, INTERVAL 1 DAY) FROM payout ORDER BY id DESC LIMIT 1");
